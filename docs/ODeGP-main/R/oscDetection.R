oscOrNot <- function(d, threshold=14, nonstat=TRUE, plotting=FALSE, Q=1) {
  ...
  res <- kernelComparison(d, altKern, plotting, Q)

  #decision on oscillatory or not
  osc.or.not <- modelSelection(res$bayesF, threshold)
  cat("==================================================\n")
...
kernelComparison <- function(d, altKern, plotting=TRUE, Q=1) {
  ...
  bayesFactor <- calculateStatistic(altMLL, nulMLL)
  res <- list("bayesF"=bayesFactor, "hypParams"<-altResult$par)
...
detrend <- function(d) {
  #' This function used to detrend the data
  #' @export
  #' @examples
  #' cat_function()
